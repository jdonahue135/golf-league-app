...
func TestForm_MinLength(t *testing.T) {
	...
	err := form.Errors.Get("some_field")
	if err == "" {
...
func TestForm_MaxLength(t *testing.T) {
	...
	form.MaxLength("some_field", 5)
	if form.Valid() {
		t.Error("shows max length of 5 met when data is shorter")
	}
	...
	form.MaxLength("another_field", 100)
	if !form.Valid() {
		t.Error("shows max length of 100 is not met when it is")
	}
